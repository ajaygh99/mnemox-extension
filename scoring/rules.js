...
const RULES = [
  ...
  {
    id: 'R1', name: 'Clarity',
    description: 'Avoids vague openers and ambiguous pronouns without clear reference.',
    check(text) {
...

  {
    ...
    description: 'Prompt is specific enough to act on. No filler phrases.',
    check(text) {
  ...
  {
    id: 'R3', name: 'Context',
    description: 'Provides background so the AI understands the situation.',
    check(text) {
      ...
      // Also check for audience/target signals which count as strong context
...

  {
    ...
    description: 'The desired output or action is explicitly stated.',
    check(text) {
      const goalSignals = [
        /\?$/m,
...

  {
    ...
    description: 'Includes format, length, audience, or style requirements.',
    check(text) {
...

  {
    ...
    description: 'Prompt has enough substance to be actionable.',
    check(text) {
      const words = text.trim().split(/\s+/).length;
      const taskCount = (text.match(/\b(and then|also|additionally|furthermore|next|finally)\b/gi) || []).length;
      // Single words or very short prompts have no structure
...

  {
    ...
    description: 'Prompt length is proportional to task complexity.',
    check(text) {
      const words = text.trim().split(/\s+/).length;
      ...
      // Single/minimal words are always imbalanced
  ...
  {
    id: 'R8', name: 'Examples',
    description: 'Bonus: includes examples to guide the AI output.',
    check(text) {
...
function scorePrompt(text) {
  if (!text || text.trim().length === 0) {
    return { score: 0, grade: 'F', dims: {}, weak: ['All'], improved: '', empty: true };
  }
  ...
  var score = Math.min(100, Math.round(total));
  var grade = score >= 85 ? 'A' : score >= 70 ? 'B' : score >= 55 ? 'C' : score >= 40 ? 'D' : 'F';

  return { score: score, grade: grade, dims: results, weak: weak };
}
